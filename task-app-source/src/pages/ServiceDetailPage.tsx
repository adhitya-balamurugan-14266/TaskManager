...
import { Plus, ArrowLeft, Archive, Bell } from 'lucide-react';

interface ServiceDetailPageProps {
  service: Service;
  state: AppState;
  onBack: () => void;
  onCreateTask: (data: { title: string; days_assigned: number; due_date?: string; reminder: boolean; description?: string; reminder_time?: string; reminder_email?: string }) => void;
  onCompleteTask: (id: string) => void;
  onDeleteTask: (id: string) => void;
  onUpdateTask: (id: string, data: { title?: string; description?: string; days_assigned?: number; due_date?: string; reminder?: boolean; reminder_time?: string; reminder_email?: string }) => void;
  userEmail: string;
}
...
export function ServiceDetailPage({
  ...
  const activeTasks = state.tasks.active.filter((t) => t.service_id === service.id);
  const overdueTasks = state.overdue.filter((t) => t.service_id === service.id);
  ...
  const filteredArchive = archiveMonth
    ? completedMonths.filter((m) => m.month === archiveMonth)
    : completedMonths;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {activeTasks.length} active · {Object.values(state.tasks.completed).flat().filter((t) => t.service_id === service.id).length} completed
      ...
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
        {(['active', 'completed', 'overdue'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
