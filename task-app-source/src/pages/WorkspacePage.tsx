...
import type { AppState, Service, ActiveTask, CompletedTask } from '@/types';
...
function TaskKanbanCard({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-md transition-all"
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <Badge variant={status}>
          {status === 'overdue' ? `${daysOverdue}d overdue` : status}
        </Badge>
        <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0 mt-0.5">{dueDateShort}</span>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div className="rounded-xl bg-gray-50 dark:bg-gray-800 px-3 py-2.5">
          ...
          <p className="text-sm font-medium text-gray-800 dark:text-gray-100">
            {task.days_assigned} day{task.days_assigned !== 1 ? 's' : ''}
          </p>
        </div>
        {selected.kind === 'completed' && (
...
export function WorkspacePage({ state, onBack, onSelectService }: WorkspacePageProps) {
  ...
  const activeTasks = state.tasks.active.filter((t) => !overdueIds.has(t.id));
  ...
  function openCard(kind: 'active' | 'completed' | 'overdue', taskId: string) {
    if (kind === 'active') {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {activeTasks.length} active · {completedTasks.length} completed · {overdueTasks.length} overdue
              </p>
            </div>
          </div>
